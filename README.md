...
### Известное ограничение детектора

При наклоне мишени больше ~30° поиск сетки иногда не сходится: он строится на
выпуклой оболочке найденных углов, и если сквозь фильтры просочилось хотя бы
несколько лишних точек, настоящий контур доски перестаёт быть наибольшим
четырёхугольником оболочки. На проверочной серии из 8 сильно наклонённых
снимков распознаётся 3.

Для калибровки это не смертельно — нераспознанные снимки пропускаются с
объяснением, и оставшихся хватает. Но лечится это не подбором порогов
(пробовал — каждый раз ломается что-то другое), а заменой поиска по оболочке на
**наращивание сетки от затравки**: взять пару соседних углов, предсказать
следующий по локальной проективной модели, найти ближайший реальный и повторять.
Такой поиск не зависит от оболочки вовсе и устойчив к любому количеству мусора.
...
1. Наращивание сетки от затравки вместо поиска по выпуклой оболочке — снимет
   ограничение детектора на сильных наклонах (см. выше).
